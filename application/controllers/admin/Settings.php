...
class Settings extends AdminController
{
    ...
    public function index()
    {
        ...
        if ($this->input->post()) {
            ...
            if ( isset($post_data['settings']['webhook_new']) ) {

                $webhooks = array(
                    "new_searches" => site_url() . $post_data['settings']['webhook_new'],
                    "modified_searches" => site_url() . $post_data['settings']['webhook_modified'],
                    "canceled_searches" => site_url() . $post_data['settings']['webhook_canceled']
                );

                $this->load->helper('api_helper');
                $register_webhooks = register_api_webhook($webhooks);

                if ($register_webhooks) {
                    update_option('register_webhook_response', serialize($register_webhooks), 'no');
                }

                update_option('webhook_new', $post_data['settings']['webhook_new'], 'no');
                update_option('webhook_modified', $post_data['settings']['webhook_modified'], 'no');
                update_option('webhook_canceled', $post_data['settings']['webhook_canceled'], 'no');
        ...
        $this->app_tabs->add_settings_tab('load', array(
            'slug' => 'load',
            'name' => 'Load Searches Data',
            'view' => 'admin/settings/includes/load',
            'position' => 60,
            'icon' => '',
            'href' => '#',
            'children' => array()
        ...
        $this->app_tabs->add_settings_tab('webhook', array(
            'slug' => 'webhook',
            'name' => 'Webhook',
            'view' => 'admin/settings/includes/webhook',
            'position' => 61,
            'icon' => '',
            'href' => '#',
            'children' => array()
