        ...
        if ( ! isset($_GET['next']) ) {
            ...
            if ( ! isset( $_GET['process'] ) ) {
                ...
                if ( $total <= $multiple ) {
                    ...
                    echo '<script>setTimeout(function(){ window.location = "/api/load_fresh_data?next=done&process=0"; }, 500);</script>';
                    exit;
                }
                ...
                if ( $process != 'completed' ) {
                    ...
                    for($i = $process; $i < $total_load; $i++) {
                        ...
                        $this->searches_model->add($searches[$i], $load_time);

                        if ( $i == ($total-1) ) {
                            echo '<script>setTimeout(function(){ window.location = "/api/load_fresh_data?next=done&process=completed"; }, 500);</script>';
                            exit;
                        }

                        if ( $i == ($total_load-1) ) {
                            echo '<script>setTimeout(function(){ window.location = "/api/load_fresh_data?next=done&process='.$total_load.'"; }, 500);</script>';
                        }
