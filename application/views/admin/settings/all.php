    ...
    <ul class="nav navbar-pills navbar-pills-flat nav-tabs nav-stacked">
      ...
      <div class="panel_s">
       <div class="panel-body">
        <a href="<?php echo admin_url('settings?group=load'); ?>" class="<?php if($this->input->get('group') == 'load'){echo 'bold';} ?>">
          Load Searches Data
        </a>
...
<script>
  var load_key = '<?php echo time(); ?>';
  function refreshSearchData(startRow = '') {
    $('.load-data-popup').fadeIn().css('display', 'flex');
    
    $.ajax({
      url: '<?php echo site_url(); ?>api/refresh_search_data',
      type: 'POST',
      dataType: 'json',
      data: {
        start_row: startRow
      ...
      success: function (response) {
        if ( response.start_row !== '' ) {
          refreshSearchData(response.start_row);
        }
        $('.load-data-popup').css('display', 'none').hide();
      }
    });
    
  }

  function loadDataAjax(next = '') {
    $('.load-data-popup').fadeIn().css('display', 'flex');

    $.ajax({
      url: '<?php echo site_url(); ?>api/load_data',
      type: 'POST',
      dataType: 'json',
      data: {
        next: next,
  ...
  $('#load-initial-data').on('click', function(e) {
    e.preventDefault();
    loadDataAjax();
  });

  <?php $this->load->helper('api_helper'); ?>
  var tables = <?php echo json_encode(ab_table_names()); ?>;
  ...
  function load_table(tableName) {
    $('.load-data-popup').fadeIn().css('display', 'flex');
    $('.load-data-popup em').html('<h3>'+tableName+'</h3>');
    return $.ajax({
        url: '<?php echo site_url(); ?>api/load_tables', 
        type: 'POST',
        dataType: 'json',
        data: {
