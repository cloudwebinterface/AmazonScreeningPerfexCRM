...
/**
 * PHP 8.4 removed strftime(); use IntlDateFormatter when available.
 */
function legacy_strftime($format, $timestamp = null)
{
    if ($timestamp === null) {
        $timestamp = time();
    ...
    if (extension_loaded('intl')) {
        $locale = 'en_US';
        if (function_exists('get_locale_key')) {
            $key = get_locale_key();
            if (!empty($key)) {
                $locale = $key;
            }
    ...
    return date(convert_strftime_to_php_date($format), $timestamp);
}

function convert_strftime_to_php_date($format)
{
    return strtr($format, [
        '%Y' => 'Y',
        '%y' => 'y',
...
function _d($date)
{
    ...
    $formatted = legacy_strftime($format, strtotime($date));
...
function _dt($date, $is_timesheet = false)
{
    ...
    if ($hour12 == false) {
        ...
        $date = legacy_strftime($format . ' ' . $tf, $date);
    } else {
...
function to_sql_date($date, $datetime = false)
{
    ...
    $d    = legacy_strftime('%Y-%m-%d %H:%M:%S', strtotime($date));
